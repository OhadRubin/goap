    ...
    def add(self, name: str, binding: str, func: Callable[[], SensorResponse]):
        if name in self._sensors:
            raise SensorAlreadyInCollectionError(
                f"Sensor '{name}' already in collection."
            )
...
class Automaton:
    ...
    def __init__(
        self, world_state: WorldState, sensors: Sensors, actions: List[RegAction]
    ):
...
class AutomatonController:
    ...
    def goal_satisfied(self) -> bool:
        ...
        for k, v in self.automaton.goal.items():
            if (
                k not in self.automaton.world_state
                or self.automaton.world_state[k] != v
            ):
                return False
        return True
...
if __name__ == "__main__":
    ...
    class FlipTable(RegAction):
        effects = {"table_id": Ellipsis}  # dynamic
        preconditions = {"has_table": True}
        cost = 2.0
